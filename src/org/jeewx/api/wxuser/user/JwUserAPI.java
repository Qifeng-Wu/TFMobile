...
import org.jeewx.api.core.req.model.user.UserInfoGet;
...
public class JwUserAPI {
	...
	/**
	 * 根据user_openid 获取未关注用户的基本信息
	 * 
	 * @param shelf_id
	 * @return
	 * @throws WexinReqException
	 */
	public static WxuserUnFocus  getWxuserUnFocus (String accesstoken,String user_openid) throws WexinReqException {
		if (accesstoken != null) {
			UserInfoGet userInfoGet = new UserInfoGet();
			userInfoGet.setAccess_token(accesstoken);
			userInfoGet.setOpenid(user_openid);
			JSONObject result = WeiXinReqService.getInstance().doWeinxinReqJson(userInfoGet);
			// 正常返回
			WxuserUnFocus wxuser = null;
			Object error = result.get("errcode");
			 
			wxuser = (WxuserUnFocus) JSONObject.toBean(result, WxuserUnFocus.class);
			return wxuser;
		}
		return null;
	}
