 ...
 */
@RestController
@RequestMapping(value = "tf-api/bj-mobile/DVTReport")
public class DVTReport {
	...
	public AjaxJson findList(HttpServletRequest request){
		...
		String sql = "SELECT type,count(*) as number FROM DVT_data WHERE type like ('%"+searchName+"%') group by type";		
	...
	public AjaxJson detail(HttpServletRequest request){
		...
		AjaxJson ajax = new AjaxJson();			
		ProductionSqlServerHelper sqlhelper = new ProductionSqlServerHelper();
		String sql = "SELECT * FROM DVT_data where type='"+type+"'";		
		List<Object> list = sqlhelper.query(sql);
		LinkedHashMap<String, Object> body = new LinkedHashMap<String, Object>();
		body.put("list", list);
		ajax.setBody(body);
